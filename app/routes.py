...
BREAKDOWN_LABELS = {
    'pre_merge_tribal': 'Pre-merge', 'post_merge_tribal': 'Post-merge',
    'finale_tribal': 'Finale',
    'jury': 'Jury', 'merge': 'Merge', 'placement': 'Placement',
    'final_tribal': 'FTC', 'individual_immunity': 'Indiv. Imm.',
    'tribal_immunity': 'Tribal Imm.', 'idols_found': 'Idols',
    'advantages_found': 'Advantages', 'idol_plays': 'Idol Plays',
    'advantage_plays': 'Adv. Plays', 'fire_win': 'Fire Win',
...
_EP_STAT_MAP = {
    ...
    'correct_votes': 'correct_votes', 'nullified': 'votes_nullified',
    'sit_outs': 'sit_outs', 'tribe': 'tribe', 'tribe_color': 'tribe_color',
}
...
def _build_leaderboard(season):
    ...
    for user in users:
        ...
        for pick in picks:
            ...
            if survivor.individual_immunity_wins:
                stats_detail.append(('Individual immunity', survivor.individual_immunity_wins))
            ...
            if survivor.tribal_councils_attended:
                stats_detail.append(('Tribals attended', survivor.tribal_councils_attended))
        ...
        if season.is_active:
            ...
            if merge_elim is not None and current_elim_count >= merge_elim:
                ...
                if not has_replacement:
                    draft_elim = any(
                        p.pick_type == 'draft' and p.survivor.voted_out_order > merge_elim
                        for p in picks
                    )
                    wc_elim = any(
                        p.pick_type == 'wildcard' and p.survivor.voted_out_order > merge_elim
                        for p in picks
                    )
                    if draft_elim:
...
def _apply_as_of(season, as_of):
    ...
    max_elim = max((orig['voted_out_order'] for orig in originals.values()
...
def rules(season_id):
    ...
    for key, (label, desc) in CONFIG_LABELS.items():
        ...
        if not progressive and key in ('tribal_base', 'tribal_step',
                                        'post_merge_step', 'finale_step',
                                        'finale_size'):
            continue
    ...
    if season and season.merge_threshold is not None:
        ...
        if progressive:
            ...
            examples['last_post_merge'] = last_pre + pm_count * pm_step if pm_count > 0 else last_pre
            ...
            examples['winner_total'] = examples['winner_tribal_pts'] + config.get('first_val', 0) + config.get('merge_val', 0) + config.get('final_tribal_val', 0)
            ...
            examples['juror_total'] = examples['juror_tribal_pts'] + config.get('jury_val', 0) + config.get('merge_val', 0)
        else:
            ...
            examples['winner_total'] = (mt * pre_merge_rate + post_merge_count * post_merge_rate
                                        + config.get('first_val', 0) + config.get('final_tribal_val', 0)
                                        + config.get('merge_val', 0))
            examples['juror_total'] = mt * pre_merge_rate + config.get('jury_val', 0) + config.get('merge_val', 0)

    return render_template('rules.html', season=season, config=config,
                           active_rules=active_rules, inactive_rules=inactive_rules,
                           progressive=progressive, examples=examples,
...
def leaderboard(season_id):
        ...
        for entry in leaderboard_data:
            for pick in entry['picks']:
                if pick.get('pick_type_raw') == 'sole_survivor':
                    continue
                if best_pick is None or pick['modified_points'] > best_pick['modified_points']:
                    best_pick = pick
                    best_pick_owner = entry['user'].display_name or entry['user'].username
    ...
    other_seasons = Season.query.filter(
        Season.id != season.id, Season.number < season.number).all()
    ...
    return render_template(
        ...
        win_pcts=win_pcts, projected_win_pcts=projected_win_pcts,
        stat_boards=stat_boards, breakdown_labels=breakdown_labels,
        ...
        is_finished=is_finished, finale_data=finale_data,
        past_winner_badges=past_winner_badges,
        breakdown_chart=breakdown_chart, confessional_chart=confessional_chart)
...
def _build_stat_boards(season):
    ...
    def top_by(attr, label, limit=10, fmt=None):
        ...
        return {
            'label': label,
            'rows': [
                {'survivor': s, 'value': fmt(getattr(s, attr)) if fmt else getattr(s, attr)}
    ...
    perf_board = top_by('performance_score', 'survivoR Performance Score',
...
def season_stats(season_id):
    season = Season.query.get_or_404(season_id)
    return render_template('stats.html', season=season, stat_boards=_build_stat_boards(season))
...
def _compare_cache_key(season):
    ...
    import hashlib
    state = '|'.join(
        f'{s.id}:{s.voted_out_order}' for s in season.survivors
    )
    picks = '|'.join(
    ...
    for preset_name, config in presets.items():
        ...
        for user in users:
            ...
            for pick in user_picks[user.id]:
                ...
                stat_overrides = None
                if pick.pick_type in ('pmr_w', 'pmr_d'):
                    stat_overrides = compute_stat_overrides(pick.survivor, merge_episode)
                modified, breakdown = scoring.score_pick(
...
def scoring_compare(season_id):
    ...
    return render_template('compare.html', season=season,
                           comparisons=data['comparisons'], labels=data['labels'],
...
def admin_picks(season_id):
    ...
    if request.method == 'POST':
        ...
        if ss_survivor and ss_episode:
            ...
            existing = SoleSurvivorPick.query.filter_by(
                user_id=target_user_id, season_id=season.id,
                episode=ss_episode_int).first()
            if existing:
                existing.survivor_id = int(ss_survivor)
            else:
                db.session.add(SoleSurvivorPick(
                    user_id=target_user_id, season_id=season.id,
                    survivor_id=int(ss_survivor), episode=ss_episode_int))
    ...
    for user in users:
        ...
        user_ss = SoleSurvivorPick.query.filter_by(
            user_id=user.id, season_id=season.id
        ).order_by(SoleSurvivorPick.episode).all()
        ss_picks_by_user[user.id] = [
            {'id': p.id, 'survivor_id': p.survivor_id, 'episode': p.episode,
...
def admin_season_detail(season_id):
    ...
    return render_template(
        ...
        default_config=DEFAULT_CONFIG, legacy_config=LEGACY_CONFIG)
...
def admin_toggle_active(season_id):
    ...
    db.session.commit()
    flash(f'Season {season.number} {"activated" if season.is_active else "deactivated"}.', 'success')
    return redirect(url_for('main.admin_seasons'))
