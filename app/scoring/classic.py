...
LEGACY_CONFIG = {
    ...
    'sole_survivor_val': 0,
    'wildcard_multiplier': 0.5, 'replacement_multiplier': 0.5,
...
CONFIG_LABELS = {
    'tribal_val': ('Pre-Merge Tribal', 'Points per pre-merge tribal (flat mode, ignored when Tribal Base is set)'),
    'post_merge_tribal_val': ('Post-Merge Tribal', 'Points per post-merge tribal (flat mode, ignored when Tribal Base is set)'),
    'tribal_base': ('Tribal Base Value', 'Starting point value for first tribal survived (enables progressive mode)'),
    'tribal_step': ('Pre-Merge Step', 'Point increase per additional pre-merge tribal survived'),
    'post_merge_step': ('Post-Merge Step', 'Point increase per additional post-merge tribal survived'),
    'finale_step': ('Finale Step', 'Point increase per additional finale tribal survived'),
    'finale_size': ('Finale Size', 'Number of players remaining when finale episode begins'),
...
class ClassicScoring(ScoringSystem):
    ...
    def _progressive_tribal_points(self, tribals_survived, season):
        ...
        post_count = min(max(0, tribals_survived - merge_threshold),
        ...
        if finale_count > 0:
            ...
            total = finale_count * last_post + f_step * finale_count * (finale_count + 1) / 2
            if total:
