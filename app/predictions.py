...
def _compute_historical_rates():
    ...
    us_cast = castaways[
        (castaways["version"] == "US") & (castaways["season"] >= NEW_ERA_START)
    ].copy()
    ...
    idol_found_counts = (
        idol_found[idol_found["advantage_id"].isin(idol_ids)]
        .groupby(["season", "castaway_id"])
        .size()
        .reset_index(name="count")
    )
    all_found_counts = (
        idol_found.groupby(["season", "castaway_id"]).size().reset_index(name="count")
    )
    adv_played = us_am[us_am["event"] == "Played"]
    played_counts = (
        adv_played.groupby(["season", "castaway_id"]).size().reset_index(name="count")
    )
    ...
    for season_num in sorted(us_cast["season"].unique()):
        ...
        n_finalists = (
            int(s_cast["finalist"].sum()) if "finalist" in s_cast.columns else 0
        )
        ...
        for _, row in s_cast.iterrows():
            ...
            # Immunity
            ii_row = ii_wins[
                (ii_wins["season"] == season_num) & (ii_wins["castaway_id"] == cid)
            ]
    ...
    us_valid["post_merge_tribals"] = (
        us_valid["tribals"] - us_valid["merge_threshold"]
    ).clip(lower=0)
    ...
    for cid in career_pm.index:
        ...
        if pm > 0:
            ii_count = (
                career_ii_total.get(cid, 0) if cid in career_ii_total.index else 0
            )
            career_ii_rates[cid] = ii_count / pm

    # --- Idol play rate: fraction of found idols that were played ---
    total_idol_found = (
        int(idol_found_counts["count"].sum()) if len(idol_found_counts) > 0 else 0
    )
    ...
    idol_play_rate = (
        total_idol_played / total_idol_found if total_idol_found > 0 else 0.5
    )

    _rates_cache = {
        "ii_per_pm_tribal": all_ii / all_post_merge_tribals
        if all_post_merge_tribals
        else 0,
    ...
    logger.info(
        ...
        n_seasons,
        _rates_cache["ii_per_pm_tribal"],
        _rates_cache["idol_per_tribal"],
        _rates_cache["adv_per_tribal"],
    ...
    for user in users_with_picks:
        ...
        for pick in user_picks[user.id]:
            ...
            if (
                ...
                and current_elim_count < merge_threshold
            ):
                continue
...
def _get_season_idol_holdings(season, remaining, idol_ids):
    ...
    if holdings:
        logger.info("Idol holdings for season %d: %s", season.number, holdings)
    return holdings
...
def calculate_win_probabilities(season):
    ...
    users_with_picks = (
        User.query.join(Pick).filter(Pick.season_id == season.id).distinct().all()
    )
    ...
    for s in survivors:
        if (
            s.voted_out_order
            and s.voted_out_order == merge_threshold
    ...
    for ordering in orderings:
        ...
        proj_ordering = _apply_idol_protection(ordering, idol_holdings, idol_play_rate)
        ...
        for surv in remaining:
            surv.made_jury = (
                surv.voted_out_order > jury_threshold
        ...
        for surv in remaining:
            ...
            ) + additional_tribals * rates["idol_per_tribal"] * rates.get(
                "idol_play_rate", 0.5
            )
    ...
    for user in users_with_picks:
        frozen_results[user.id] = {
            ...
            "win_pct": frozen_counts[user.id] / total_scenarios * 100
            if total_scenarios
            else 0,
        }
        projected_results[user.id] = {
            ...
            "win_pct": projected_counts[user.id] / total_scenarios * 100
            if total_scenarios
            else 0,
        }

    frozen_results = dict(
        sorted(frozen_results.items(), key=lambda x: x[1]["win_pct"], reverse=True)
    )
    projected_results = dict(
        sorted(projected_results.items(), key=lambda x: x[1]["win_pct"], reverse=True)
    )

    # Cache (without user objects)
    cache_frozen = {
        uid: {k: v for k, v in d.items() if k != "user"}
