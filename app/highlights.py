...
def generate_highlights(survivor, season, merge_episode, as_of_episode=None):
    ...
    events = []
    episodes = sorted(int(k) for k in ep_stats.keys())
    ...
    for ep in episodes:
        ...
        if idol_play_delta > 0:
            ...
            detail = f'nullified {nullified_delta} vote{"s" if nullified_delta != 1 else ""}' if nullified_delta > 0 else None
...
def _generate_badges(events, total_ii, total_votes_survived):
    ...
    if idol_found:
        badges.append(Badge(f'Idol Found' if idol_found == 1 else f'{idol_found}x Idol Found', IDOL))
    if idol_played:
        badges.append(Badge(f'Idol Played' if idol_played == 1 else f'{idol_played}x Idol Played', IDOL))
    ...
    if total_votes_survived > 0:
        badges.append(Badge(f'Survived {total_votes_survived} Vote{"s" if total_votes_survived != 1 else ""}', VOTES))

    # Priority 5: Advantage found/played
    adv_count = sum(1 for e in events if e.event_type == ADVANTAGE)
    if adv_count:
        badges.append(Badge(f'{adv_count}x Advantage' if adv_count > 1 else 'Advantage', ADVANTAGE))
...
def _event_type_order(event_type):
    ...
    order = {
        TRIBE: 0, MERGE: 1, IMMUNITY: 2, TRIBAL_IMMUNITY: 3,
        IDOL: 4, ADVANTAGE: 5, VOTES: 6, FIRE: 7,
        JURY: 8, ELIMINATION: 9, WINNER: 10,
    }
