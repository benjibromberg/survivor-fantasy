...
from analyze_scoring import (
    expand_config, stratified_random_sample, SimSurvivor, SimSeason, SimPick,
    calculate_leaderboard, _build_elim_to_episode, _compute_sim_stat_overrides,
    _build_tribal_table, _fast_score_total,
    evaluate_config, rank_users, PARAM_GRID, build_chart_data,
    assign_extra_picks, compute_ss_streaks, snake_draft,
)
...
def make_season(num_players=18, left_at_jury=8, n_finalists=3):
    ...
    return SimSeason(
        number=50, name='Test Season', num_players=num_players,
...
class TestExpandConfig:
    ...
    def test_progressive_mode_removes_flat_keys(self):
        config = {
            'tribal_base': 1.0, 'tribal_step': 0.5,
            'tribal_val': 0.5, 'post_merge_tribal_val': 1.0,
        }
        result = expand_config(config)
...
class TestCalculateLeaderboard:
    ...
    def test_as_of_zero_all_zero(self):
        ...
        ss = {1: 0, 2: 0}
        lb = calculate_leaderboard(season, scoring, season.survivors, picks, ss, as_of=0)
    ...
    def test_as_of_excludes_future_eliminations(self):
        ...
        lb = calculate_leaderboard(season, scoring, season.survivors, picks, ss, as_of=1)
    ...
    def test_elim_to_episode_passed_through(self):
        ...
        lb1 = calculate_leaderboard(
            season, scoring, season.survivors, picks, ss,
            elim_to_episode=elim_to_ep)
        lb2 = calculate_leaderboard(
            season, scoring, season.survivors, picks, ss)
        assert lb1 == lb2
...
class TestMetrics:
    ...
    def test_high_tribal_config_has_high_longevity(self):
        ...
        result = self._run_evaluate({
            'tribal_base': 2, 'tribal_step': 1,
            'post_merge_step': 1.5, 'finale_step': 2,
            'individual_immunity_val': 0, 'tribal_immunity_val': 0,
            'idol_found_val': 0, 'advantage_found_val': 0,
            'idol_play_val': 0, 'advantage_play_val': 0,
            'jury_val': 0, 'merge_val': 0,
...
class TestBuildElimToEpisode:
    def test_basic(self):
        survivors = [
            SimSurvivor(id=1, name='A', voted_out_order=1, made_jury=False,
                        elimination_episode=2),
            SimSurvivor(id=2, name='B', voted_out_order=2, made_jury=False,
...
class TestLeaderboardAsOfEdgeCases:
    ...
    def test_as_of_past_max_elim(self):
        ...
        lb_past = calculate_leaderboard(
            season, scoring, season.survivors, picks, ss,
            as_of=100, include_ss=True)
        assert lb_final == lb_past

    def test_monotonic_scores(self):
        """Scores should generally increase or stay flat as as_of increases."""
        season = make_season(num_players=6, left_at_jury=3)
        scoring = ClassicScoring(tribal_val=1, first_val=0, merge_val=0,
                                 jury_val=0, individual_immunity_val=0,
                                 tribal_immunity_val=0, idol_found_val=0,
                                 advantage_found_val=0, idol_play_val=0,
                                 advantage_play_val=0, final_tribal_val=0,
                                 fire_win_val=0)
...
class TestStatOverrideEdgeCases:
    ...
    def test_missing_keys_default_zero(self):
        ...
        surv = SimSurvivor(
            id=1, name='Test', voted_out_order=15, made_jury=True,
            individual_immunity_wins=3, idols_found=2,
...
class TestBuildTribalTable:
    def test_flat_mode(self):
        ...
        config = {**DEFAULT_CONFIG, 'tribal_val': 1, 'post_merge_tribal_val': 2,
                  'tribal_base': None}
    ...
    def test_progressive_mode(self):
        ...
        season = make_season(num_players=18, left_at_jury=8)
        config = {**DEFAULT_CONFIG, 'tribal_base': 1, 'tribal_step': 0.5,
                  'post_merge_step': 1, 'finale_step': 2, 'finale_size': 5}
        table = _build_tribal_table(config, season)
        assert table[0] == (0.0, 0.0)
        # k=1: base = 1
    ...
    def test_matches_scoring_system(self):
        ...
        season = make_season(num_players=18, left_at_jury=8)
        config = {**DEFAULT_CONFIG, 'tribal_base': 1.5, 'tribal_step': 0.25,
                  'post_merge_step': 0.5, 'finale_step': 1.5, 'finale_size': 5}
        table = _build_tribal_table(config, season)
        scoring = ClassicScoring(**config)
        for k in range(0, 18):
            ...
            assert abs(table[k][1] - expected_pre) < 1e-9, f'Pre-merge mismatch at k={k}'
...
class TestFastScoreTotal:
    ...
    def test_first_boot_zero(self):
        ...
        config, season, table = self._make_config_and_table(
            tribal_base=1, tribal_step=0.5, first_val=0, merge_val=0,
            jury_val=0, final_tribal_val=0, fire_win_val=0,
            individual_immunity_val=0, tribal_immunity_val=0,
            idol_found_val=0, advantage_found_val=0,
            idol_play_val=0, advantage_play_val=0)
    ...
    def test_flat_mode_matches_score_pick(self):
        ...
        config = {**DEFAULT_CONFIG, 'tribal_val': 1.5, 'post_merge_tribal_val': 3,
                  'tribal_base': None}
        season = make_season(num_players=18, left_at_jury=8)
        table = _build_tribal_table(config, season)
        scoring = ClassicScoring(**config)
...
class TestTimelineFidelity:
    ...
    def _make_timeline_season(self):
        ...
        season = SimSeason(
            number=99, name='Timeline Test', num_players=num_players,
            left_at_jury=left_at_jury, n_finalists=3, survivors=survivors,
        )
        return season
    ...
    def test_forward_walk_matches_as_of(self):
        ...
        for step in range(1, max_elim + 1):
            lb = calculate_leaderboard(
                ...
                as_of=step, include_ss=False, elim_to_episode=elim_to_ep)
...
class TestAssignExtraPicks:
    ...
    def test_wildcard_not_from_own_roster(self):
        ...
        assign_extra_picks(season, picks_by_user, rng)
        for pid in picks_by_user:
            own_draft_ids = {p.survivor.id for p in picks_by_user[pid]
    ...
    def test_replacement_given_when_draft_eliminated_pre_merge(self):
        ...
        season = make_season(num_players=12, left_at_jury=5)
        merge = season.merge_threshold  # 7
        # Player 0 has a draft pick eliminated at position 3 (pre-merge)
        picks_by_user = {
            0: [SimPick(season.survivors[2], 'draft'),   # elim 3, pre-merge
    ...
    def test_replacement_pmr_w_when_only_wildcard_eliminated(self):
        ...
        picks_by_user = {
            0: [SimPick(season.survivors[10], 'draft'),   # elim 11, post-merge
                SimPick(season.survivors[2], 'wildcard')],  # elim 3, pre-merge
        }
        rng = random.Random(42)
        assign_extra_picks(season, picks_by_user, rng)
    ...
    def test_no_replacement_when_no_pre_merge_loss(self):
        ...
        picks_by_user = {
            0: [SimPick(season.survivors[10], 'draft'),    # elim 11
                SimPick(season.survivors[9], 'draft'),     # elim 10
                SimPick(season.survivors[7], 'wildcard')],  # elim 8, post-merge
        }
        rng = random.Random(42)
...
class TestComputeSsStreaks:
    ...
    def test_no_winner_all_zero(self):
        ...
        season = SimSeason(number=1, name='No Winner', num_players=10,
                           left_at_jury=4, n_finalists=3, survivors=survivors)
...
class TestCheckpointAndExport:
    ...
    def _make_results(self):
        ...
        configs = [
            {'tribal_base': 1, 'tribal_step': 0.5, 'post_merge_step': 1,
             'finale_step': 2, 'first_val': 10, 'placement_ratio': (0.5, 0.2)},
            {'tribal_base': None, 'tribal_step': 0, 'post_merge_step': 0,
             'finale_step': 0, 'first_val': 15, 'placement_ratio': (0.5, 0.2)},
        ]
        results = []
    ...
    def test_build_chart_data_structure(self):
        ...
        results = self._make_results()
        export = build_chart_data(results, timelines=[])
        for key in ['total_configs', 'param_impact', 'histogram',
                    'top10', 'recommended', 'timelines']:
