...
def load_all_seasons(season_numbers=None):
    ...
    if invalid:
        raise ValueError(
            f"Only new-era seasons (41+) are supported. Invalid: {invalid}"
        )
    ...
    fire_winner_set = set(
        zip(fire_rows["season"], fire_rows["castaway_id"], strict=False)
    )

    # Pre-compute stats grouped by (season, castaway_id)
    indiv_imm = (
        us_cr[us_cr["won_individual_immunity"] == 1]
        .groupby(["season", "castaway_id"])
        .size()
    )
    tribal_imm = (
        us_cr[us_cr["won_tribal_immunity"] == 1]
        .groupby(["season", "castaway_id"])
        .size()
    )
    ...
    for snum in season_numbers:
        ...
        season_name = (
            str(ss["season_name"]) if pd.notna(ss["season_name"]) else f"Season {snum}"
        )
        ...
        if not s_am_season.empty:
            ...
            idols_found_s = idols_played_s = adv_found_s = adv_played_s = pd.Series(
                dtype=int
            )
        ...
        s_idol_found_by_ep = (
            _season_ep_counts(
                s_am_season,
                lambda df, _ids=s_idol_ids: df[
                    df["event"].str.contains("Found", na=False)
                    & df["advantage_id"].isin(_ids)
                ],
            )
            if not s_am_season.empty
            else {}
        ...
        s_idol_play_by_ep = (
            _season_ep_counts(
                s_am_season,
                lambda df, _ids=s_idol_ids: df[
                    (df["event"] == "Played") & df["advantage_id"].isin(_ids)
                ],
            )
            if not s_am_season.empty
            else {}
        ...
        for _, row in cast.iterrows():
            ...
            survs.append(
                SimSurvivor(
                    ...
                    made_jury=made_jury,
                    individual_immunity_wins=ii,
                    tribal_immunity_wins=ti,
                    idols_found=idf,
                    idols_played=ipd,
                    advantages_found=af,
                    advantages_played=ap,
...
def assign_extra_picks(season, picks_by_user, rng):
    ...
    post_merge_alive = [
        ...
        if s.voted_out_order == 0 or s.voted_out_order > merge_thresh
    ]
    for pid in picks_by_user:
...
def _build_tribal_table(config, season):
    ...
    for k in range(1, season.num_players + 1):
        if use_progressive:
            ...
            post_count = min(
                max(0, k - merge_threshold), max(0, finale_threshold - merge_threshold)
            )
            ...
            if post_count > 0:
                ...
                post_merge = (
                    post_count * last_pre + pm_step * post_count * (post_count + 1) / 2
                )
            else:
                post_merge = 0.0
...
def _fast_score_total(
    survivor, config, season, tribal_table, stat_overrides=None, is_replacement=False
):
...
def _build_elim_to_episode(survivors):
    ...
    return {
        s.voted_out_order: s.elimination_episode
        for s in survivors
...
def calculate_leaderboard(
    ...
    for user_id, picks in picks_by_user.items():
        ...
        for pick in picks:
            ...
            if (
                pick.pick_type in ("pmr_w", "pmr_d")
                and as_of is not None
                and as_of < merge_thresh
            ):
                continue
...
def evaluate_config(config, scenarios):
    ...
    for season, survivors, picks_by_user, _ss_streaks, max_elim, n_players in scenarios:
        ...
        for user_id, picks in picks_by_user.items():
            for pick in picks:
                if (
                    pick.survivor.voted_out_order
                    and 1 <= pick.survivor.voted_out_order <= 3
                ):
        ...
        if leaders and winner_id is not None:
            not_leading = sum(1 for leader in leaders if leader != winner_id)
        ...
        for _user_id, pick_results in final_breakdowns.items():
        ...
        for user_id, picks in picks_by_user.items():
            ...
            valid_draft = [p for p in draft_picks if p.survivor.voted_out_order]
            if valid_draft:
                avg_order = sum(p.survivor.voted_out_order for p in valid_draft) / len(
                    valid_draft
                )
        ...
        if len(draft_quality) >= 3 and final_ranking:
            ...
            quality_rank = sorted(
                draft_quality, key=lambda u: draft_quality[u], reverse=True
            )
            q_rank_map = {uid: i for i, uid in enumerate(quality_rank)}
            f_rank_map = {
                uid: i for i, uid in enumerate(final_ranking) if uid in q_rank_map
            }
            n_r = len(f_rank_map)
            if n_r >= 3:
                d_sq_sum = sum(
    ...
    result["composite"] = (
        result.get("draft_skill_correlation", 0)
        * 2.0  # good drafting wins (higher = better)
...
def print_metrics(metrics):
    print(
        f"  Draft skill corr:     {metrics.get('draft_skill_correlation', 0):.3f}"
        f"  (good drafting → winning, weight=2.0)"
    )
    print(
        f"  Longevity share:      {metrics.get('longevity_share', 0):.0%}  (% pts from tribals, weight=2.0)"
    )
    print(
        f"  Rank volatility:      {metrics.get('rank_volatility', 0):.3f}  (avg rank change per player per step)"
    )
    print(
        f"  Comeback rate:        {metrics.get('comeback_rate', 0):.0%}  (midpoint leader loses)"
    )
    print(
        f"  Suspense:             {metrics.get('suspense', 0):.0%}  (steps where winner not leading)"
    )
    print(f"  Lead changes:         {metrics.get('lead_changes', 0):.2f}  (per step)")
    print(
        f"  Early loser rank:     {metrics.get('early_loser_avg_rank', 0):.2f}  (0-1, lower = better recovery)"
    )
    print(
        f"  Midpoint competitive: {metrics.get('midpoint_competitive_pct', 0):.0%}  (within 20% of lead)"
    )
    print(
        f"  Late-game gap:        {metrics.get('late_game_gap', 0):.0%}  (1st-2nd gap at 75%)"
    )
    print(
        f"  Final spread:         {metrics.get('final_spread', 0):.0%}  (1st-last gap)"
    )
    print(
        f"  Blowout rate:         {metrics.get('blowout_rate', 0):.0%}  (1st > 2x last)"
    )
    print(
        f"  Non-draft impact:     {metrics.get('non_draft_pct', 0):.0%}  (pts from wildcard/SS)"
    )
    ...
    print(
        f"  Consistency penalty:  {penalty:.3f}"
...
def build_season_timelines(seasons, recommended_config, seed=99):
    ...
    for season in seasons:
        ...
        datasets = []
        for uid in sorted(
            picks_by_user.keys(), key=lambda u: series[u][-1], reverse=True
        ):
            datasets.append(
                {
                    "name": f"Player {uid + 1}",
        ...
        timelines.append(
            {
                ...
                "labels": [
                    "Sole Survivor"
                    if s == season.num_players
                    else (f"Elim {s}" if s > 0 else "Start")
                    for s in steps
                ],
                "datasets": datasets,
            }
...
def build_comparison_timelines(seasons, recommended_config, n_candidates=50, seed=42):
    ...
    for season in seasons:
        ...
        for _ in range(n_candidates):
            ...
            for step_idx in range(len(series_rec[winner_uid])):
                step_scores = {uid: series_rec[uid][step_idx] for uid in series_rec}
                leaders.append(max(step_scores, key=step_scores.get))
            not_leading = sum(1 for leader in leaders if leader != winner_uid)
        ...
        sorted_uids = sorted(
            picks_by_user.keys(), key=lambda u: series_rec[u][-1], reverse=True
        )

        def make_datasets(series, _sorted_uids=sorted_uids):
            return [
                {"name": f"Player {uid + 1}", "points": series[uid]}
                for uid in _sorted_uids
            ]
...
def build_season_health_stats(recommended_config, scenarios):
    ...
    for season, survivors, picks_by_user, _ss_streaks, max_elim, n_players in scenarios:
        ...
        for s in survivors:
            if s.id in orig_state:
                s.voted_out_order, s.made_jury, s.won_fire, saved, s.day_voted_out = (
                    orig_state[s.id]
                )
...
def build_chart_data(
    ...
    for i, (config, metrics) in enumerate(results):
        expanded = expand_config(config)
        if all(
            expanded.get(k) == LEGACY_CONFIG.get(k)
            for k in LEGACY_CONFIG
            if k in expanded
        ):
            legacy_rank = i + 1
...
def main():
    ...
    parser.add_argument(
        "--export-json", type=str, default=None, help="Export chart JSON to this path"
    )
    parser.add_argument(
        "--cores",
        type=int,
    ...
    if args.reexport:
        ...
        legacy_cfg = {
            k: LEGACY_CONFIG.get(k, DEFAULT_CONFIG.get(k))
            for k in PARAM_GRID
            if k in LEGACY_CONFIG or k in DEFAULT_CONFIG
        }
    ...
    t0 = time.time()
    print(
        f"Loading survivor data for {len(season_numbers)} seasons from {SURVIVOR_DATA_FILE}..."
    )
    seasons = load_all_seasons(season_numbers)
    print(f"Loaded {len(seasons)} complete seasons in {time.time() - t0:.1f}s")

    print(
        f"Generating draft scenarios ({len(PLAYER_COUNTS)} sizes x {drafts_per} drafts x {len(seasons)} seasons)..."
    )
    scenarios = generate_scenarios(seasons, drafts_per=drafts_per)
    print(
        f"Generated {len(scenarios)} draft scenarios "
    ...
    print(f"\n{'=' * 80}")
    print(
        f"PARAMETER SWEEP ({n_samples} configs via stratified sampling, {len(scenarios)} scenarios each)"
    )
    print(f"{'=' * 80}")
    ...
    configs.append(
        {k: DEFAULT_CONFIG.get(k) for k in PARAM_GRID if k in DEFAULT_CONFIG}
    )
    ...
    print(
        f"Evaluating {len(configs)} configs across {n_workers} CPU cores ({mp.cpu_count()} available)..."
    )
    ...
    with mp.Pool(n_workers, initializer=_init_worker, initargs=(scenarios,)) as pool:
        for i, result in enumerate(pool.imap_unordered(_evaluate_worker, configs)):
            ...
            if done % 200 == 0 or done == len(configs):
                ...
                print(
                    f"  {done}/{len(configs)} evaluated  ({rate:.1f}/sec, ~{eta:.0f}s remaining)"
                )
    ...
    print(f"\n{'=' * 80}")
    print(
        f"PHASE 2: NEIGHBORHOOD REFINEMENT (top {top_n}, ±{step_range} steps, {n_rounds} rounds)"
    )
    print(f"{'=' * 80}")
    ...
    for round_num in range(1, n_rounds + 1):
        ...
        print(
            f"  Round {round_num}: evaluating {len(neighbor_configs)} neighbor configs..."
        )
        t_refine = time.time()
        with mp.Pool(
            n_workers, initializer=_init_worker, initargs=(scenarios,)
        ) as pool:
            for i, result in enumerate(
                ...
                if done % 200 == 0 or done == len(neighbor_configs):
                    ...
                    print(
                        f"    {done}/{len(neighbor_configs)} evaluated  ({rate:.1f}/sec, ~{eta:.0f}s remaining)"
                    )
        results.sort(key=lambda x: x[1]["composite"], reverse=True)
        new_in_top10 = sum(
            1 for c, _ in results[:10] if tuple(sorted(c.items())) not in prev_top10
        )
    ...
    checkpoint = [{"config": config, "metrics": metrics} for config, metrics in results]
    ...
    for param, label, unit in [
        ...
        print(f"\n  {label} ({unit}):")
        sorted_keys = sorted(groups.keys(), key=lambda x: (isinstance(x, bool), x))
